...
class AuthViewModel @Inject constructor(private val useCases: UseCases) : ViewModel() {
    ...
    private val _user = mutableStateOf(Owner())
    ...
    fun getUserByEmail(email: String) {
        ...
        viewModelScope.launch {
            useCases.getOwnerByEmail(email).collect { response ->
                _user.value = response; response.id?.let {
                Log.d(
                    "getUserResponse", it
                )
            }
        ...
        auth.createUserWithEmailAndPassword(_userEmail.value, _password.value)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    viewModelScope.launch {
                        useCases.addOwner(
                            name.value, userEmail.value, phone.value, address.value, website.value
                        ).collect {}
    ...
    fun isValidEmailAndPassword(): Boolean {
        return (userEmail.value.isNotBlank() && password.value.isNotBlank())
    }
